...
#alpha_c = 0.  # regularization parameter for 'doubly stochastic attention', as in the paper
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, data_name, 'TRAIN'),batch_size=batch_size ,shuffle=True, num_workers=workers, pin_memory=True)
    val_loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, data_name, 'VAL'),batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)
...
def train(train_loader, decoder, criterion_ce, decoder_optimizer, epoch):
    ...
    # Batches
    for i, (img_att,img_fc, caps, caplens) in enumerate(train_loader):
        ...
        scores, scores_d, caps_sorted, decode_lengths, alphas, sort_ind = decoder(img_att,img_fc, caps, caplens)
        ...
        if i % print_freq == 0:
            ...
            info = {'train loss':loss.item(),'train top5 accuracy':top5}
...
def validate(val_loader, decoder, criterion_ce):
    ...
    # Batches
    for i, (img_att,img_fc, caps, caplens, allcaps) in enumerate(val_loader):
        ...
        if i % 30 == 0:
            ...
            info = {'val loss':loss.item(),'val top5 accuracy':top5}
            for tag,value in info.items():
                logger.scalar_summary(tag,value,i)
